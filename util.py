...
def load_prediction(split, name, mode='fulltrain'):
    ...
    if type(name) is list:
        ...
        if split == 'test' and os.path.exists('preds/%s-%s.csv' % (name, 'test-%s' % mode)):
            return pd.read_csv('preds/%s-%s.csv' % (name, 'test-%s' % mode), index_col='Loan_ID').iloc[:, 0]

        return pd.read_csv('preds/%s-%s.csv' % (name, split), index_col='Loan_ID').iloc[:, 0]
...
class Dataset(object):
    ...
    part_types = {
        ...
        'numeric_lin': 'd2',
        'numeric_mean_scaled': 'd2',
        'numeric_mean_boxcox': 'd2',
        'numeric_mean_rank_norm': 'd2',
